...
class ControlsBar extends Component {
  getActionIcon = () => {
    const { isPlaying, isComplete } = this.props;
  ...
  renderAction = () => {
    const { isPlaying, isComplete, pause, play } = this.props;

    const ActionIcon = this.getActionIcon();

    return (
      <ControlContainer className="PlayAndPause" onClick={isPlaying && !isComplete ? pause : play}>
        <Icon iconName={ActionIcon} size="1.2em" />
      </ControlContainer>
    );
  };
  ...
  renderTime = () => {
    ...
    return <TimeContainer>{`${currentTime} / ${totalTime}`}</TimeContainer>;
  };
  ...
  renderVolume = () => {
    ...
    return (
      <ControlContainer className="Sound" onClick={isMuted ? unmute : mute}>
        <Icon iconName={VolumeIcon} size="1.2em" />
      </ControlContainer>
    );
  };

  renderVolumeBar = () => {
    const { volume, updateVolumeTo } = this.props;
    return (
      <ControlContainer>
        <RangeBar currentValue={volume} setTo={updateVolumeTo} size="0.15em" />
      </ControlContainer>
    );
  };

  renderFullScreenToggle = () => {
    const { enterFullscreenMode, exitFullscreenMode, isFullscreenMode } = this.props;
    ...
    return (
      ...
      >
        <Icon iconName={isFullscreenMode ? 'FullscreenExitIcon' : 'FullscreenIcon'} size="1.2em" />
      </ControlContainer>
    );
  };
  ...
  render() {
    ...
    return (
      <ControlsBarContainer>
        ...
        <RightControlsContainer>{this.renderFullScreenToggle()}</RightControlsContainer>
      </ControlsBarContainer>
    );
  }
}
